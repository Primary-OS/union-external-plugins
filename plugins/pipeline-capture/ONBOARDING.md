...
- **Optional (cloud routine only) — audited crypto:** `publish` encrypts with a bundled pure-Python sealed box by default (no install, works offline and behind blocked egress), so nothing is required here. If you'd rather the routine use the audited PyNaCl binding, paste `setup.sh` (in this plugin's root) into the routine's **Cloud Environment → Setup script**; it `pip install`s PyNaCl once at build (cached). Requires the environment's **Network access = Trusted** (PyPI is default-allowlisted). If PyPI is blocked it no-ops and the pure-Python fallback is used — publish still works.
